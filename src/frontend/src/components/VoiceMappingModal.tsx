    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-ark-border">
          <div>
            <h2 className="text-lg font-bold text-ark-white">{t('mapping.title')}</h2>
        ...
        <div className="px-4 py-2 bg-ark-black/30 border-b border-ark-border flex items-center justify-between">
          <span className="text-xs text-ark-gray">
            {t('mapping.info.voicelessCount', { count: voicelessCharacters.length })}
          </span>
          <span className={`text-xs ${mappedCount > 0 ? 'text-green-400' : 'text-ark-gray'}`}>
            {t('mapping.info.mappedCount', { mapped: mappedCount, total: voicelessCharacters.length })}
          </span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {isLoadingMeta ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-ark-white">{t('mapping.message.allHaveVoice')}</p>
            </div>
          ) : (
        ...
        <div className="p-4 border-t border-ark-border flex justify-between items-center">
          <div className="text-xs text-ark-gray/70 space-y-0.5">
            <p>{t('mapping.note.autoAssignment')}</p>
            <p>{t('mapping.note.autoSave')}</p>
          </div>
          <button
            onClick={onClose}
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        <button
          ...
          title={autoVoiceName ? t('mapping.tooltip.auto', { name: autoVoiceName }) : undefined}
        >
          {t('mapping.button.auto')}
        </button>
        {hasDefaultFemale && (
          <>
            <button
              ...
              title={autoFemaleName ? t('mapping.tooltip.female', { name: autoFemaleName }) : undefined}
            >
              ♀
            </button>
            <button
              ...
              title={t('mapping.tooltip.randomFemale')}
            >
              ♀🎲
            </button>
          </>
        ...
        {hasDefaultMale && (
          <>
            <button
              ...
              title={autoMaleName ? t('mapping.tooltip.male', { name: autoMaleName }) : undefined}
            >
              ♂
            </button>
            <button
              ...
              title={t('mapping.tooltip.randomMale')}
            >
              ♂🎲
            </button>
          </>
        ...
        <button
          ...
          title={t('mapping.tooltip.search')}
        >
      ...
      {showSearch && (
        <div className="bg-ark-black/60 rounded border border-ark-border p-2.5 space-y-2">
          <input
            ...
            placeholder={t('mapping.placeholder.search')}
            className="ark-input text-sm w-full py-1.5 px-2.5"
          />
          <div className="max-h-40 overflow-y-auto space-y-1">
            {isSearching ? (
              <div className="text-xs text-ark-gray text-center py-2">{t('common.searching')}</div>
            ) : filteredOptions.length === 0 ? (
              <div className="text-xs text-ark-gray text-center py-2">{t('common.noSearchResults')}</div>
            ) : (
              filteredOptions.map(opt => {
                ...
                return (
                  ...
                  >
                    ...
                    {isFromApi && <span className="text-yellow-500/70 text-[10px]">{t('mapping.tag.table')}</span>}
                    {!isPrepared && !isFromApi && <span className="text-ark-gray/50 text-[10px]">{t('mapping.tag.default')}</span>}
                  </button>
                )
              })
            )}
    ...
    <div className="rounded-lg bg-ark-black/40 hover:bg-ark-black/50 transition-colors overflow-hidden border border-ark-border/50">
      <div className="flex">
        ...
        <div className="flex gap-2 p-3 bg-ark-black/30">
          ...
          <CharacterStanding
            charId={mappedCharId}
            alt={mappedCharName ?? t('mapping.status.mappingRequired')}
            className="w-28 h-44 rounded"
            showPlaceholder={true}
          />
        </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {isSaved && (
                <span className="text-xs px-1.5 py-0.5 rounded bg-green-500/20 text-green-400 flex items-center gap-0.5">
                  ...
                  {t('mapping.status.saved')}
                </span>
              )}
            </div>
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-xs text-ark-gray">{t('character.dialogueCount', { count: char.dialogue_count })}</span>
              {char.char_id && (
              ...
              {isSaved && (
                <div className="flex gap-1.5 shrink-0">
                  <button
                    ...
                    title={t('mapping.button.resetTooltip')}
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
