...
arkprts 서버 코드, 표준 로케일 코드, 단축 코드 간 변환을 제공합니다.
"""

# 표준 로케일 코드 → arkprts 서버 코드
...
def locale_to_server(locale: str) -> str:
    """표준 로케일 코드를 arkprts 서버 코드로 변환

    Args:
        locale: 표준 로케일 코드 (ko_KR, en_US, ja_JP, zh_CN)

    Returns:
        arkprts 서버 코드 (kr, en, jp, cn)
