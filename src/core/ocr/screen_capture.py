...
class ScreenCapture:
    ...
    def get_windows(self) -> list[WindowInfo]:
        ...
        def enum_callback(hwnd, _):
            ...
            try:
                ...
                error_count += 1
                if error_count <= 3:
                    logger.warning("윈도우 열거 오류: %s", e)
            return True

        try:
            win32gui.EnumWindows(enum_callback, None)
            logger.debug("윈도우 %d개 발견 (오류: %d개)", len(windows), error_count)
        except Exception as e:
            logger.exception("EnumWindows 실패: %s", e)
    ...
    def capture_window(self, hwnd: int) -> Image.Image | None:
        ...
        try:
            ...
            logger.error("Window capture error: %s", e)
            return None
...
def get_subtitle_region(screen_width: int, screen_height: int) -> BoundingBox:
    ...
    """
    margin_x = int(screen_width * 0.01)  # 좌우 1% 여백
    y_start = int(screen_height * 0.35)  # 상단 35%부터 (헤더 15% 감안)
    y_end = int(screen_height * 0.75)    # 75%까지
